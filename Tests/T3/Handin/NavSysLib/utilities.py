...
def gdop(H: Optional[np.ndarray]=None, M: Optional[np.ndarray]=None) -> Optional[float]:
    """Calculate GDOP from design matrix H or covariance matrix M."""
    if H is not None:
        try:
...
def pdop(H: Optional[np.ndarray]=None, M: Optional[np.ndarray]=None) -> Optional[float]:
    """Calculate PDOP from design matrix H or covariance matrix M."""
    if H is not None:
        try:
            HtH_inv = np.linalg.inv(H.T @ H)
            return np.sqrt(HtH_inv[0, 0] + HtH_inv[1, 1] + HtH_inv[2, 2])
        except np.linalg.LinAlgError:
            return None
    elif M is not None:
...
def tdop(H: Optional[np.ndarray]=None, M: Optional[np.ndarray]=None) -> Optional[float]:
    """Calculate TDOP from design matrix H or covariance matrix M."""
    if H is not None:
        try:
            HtH_inv = np.linalg.inv(H.T @ H)
            return np.sqrt(HtH_inv[3, 3])
        except np.linalg.LinAlgError:
            return None
    elif M is not None:
        return np.sqrt(M[3, 3])
    else:
        return None
    
def hdop(H: Optional[np.ndarray]=None, M: Optional[np.ndarray]=None) -> Optional[float]:
    """Calculate HDOP from design matrix H or covariance matrix M."""
    if H is not None:
        try:
            HtH_inv = np.linalg.inv(H.T @ H)
            return np.sqrt(HtH_inv[0, 0] + HtH_inv[1, 1])
        except np.linalg.LinAlgError:
            return None
    elif M is not None:
        return np.sqrt(M[0, 0] + M[1, 1])
    else:
        return None

def vdop(H: Optional[np.ndarray]=None, M: Optional[np.ndarray]=None) -> Optional[float]:
    """Calculate VDOP from design matrix H or covariance matrix M."""
    if H is not None:
        try:
            HtH_inv = np.linalg.inv(H.T @ H)
            return np.sqrt(HtH_inv[2, 2])
        except np.linalg.LinAlgError:
            return None
    elif M is not None:
        return np.sqrt(M[2, 2])
    else:
        return None 
    
def least_squares(H, z):
    """
    Ordinary Least Squares (LS) estimator
    ...
    Returns:
        x_hat : (n,) estimated state vector
    """
    H = np.asarray(H)
    z = np.asarray(z)

    x_hat = np.linalg.inv(H.T @ H) @ H.T @ z
    return x_hat

def weighted_least_squares(H, z, Cn):
    """
    ...
    Parameters:
        H  : (m, n) design matrix
        z  : (m,) measurements
        Cn : (m, m) noise covariance matrix
    ...
    x_hat = np.linalg.inv(H.T @ Q @ H) @ H.T @ Q @ z
    return x_hat

def get_position_from_pseudoranges(sat_positions, pseudoranges, initial_guess, num_iter=1, tol=1e-6):
    """
    Estimate receiver position from satellite positions and pseudoranges using iterative least squares.
    Iterate for num_iter iterations or until convergence within tol.

    Parameters:
        sat_positions : (m, 3) array of satellite ECEF positions
        pseudoranges  : (m,) array of pseudorange measurements
        initial_guess : (4,) initial guess for receiver state (x, y, z, clock_bias)
        num_iter      : maximum number of iterations
        tol           : convergence tolerance

    Returns:
        receiver_state : (4,) estimated receiver state (x, y, z, clock_bias)
    """

    receiver_state = np.asarray(initial_guess)

    for _ in range(num_iter):
        x, y, z, clock_bias = receiver_state
        ranges = np.sqrt(np.sum((sat_positions - receiver_state[:3])**2, axis=1)) + clock_bias
        residuals = pseudoranges - ranges
        ...
        delta_state = least_squares(H, residuals)
        receiver_state += delta_state

        if np.linalg.norm(delta_state) < tol:
            break
